  ...
  return (
    <Wrapper>
      {(() => {
        ...
        if (error) return <CardBody>`<FormattedMessage defaultMessage="Error!"/> ${error.message}`</CardBody>;
        const searchResults = [
          ...(data.needs || []),
        ...
        if (searchResults.length === 0) return <CardBody><FormattedMessage defaultMessage="No results" /></CardBody>;
        return (
          <SearchResults
            results={_.orderBy(
              searchResults,
